...
async function generateChangelog(newCommits) {
  const cohere = sdk.model("cohere/command-r", process.env.COHERE_API_KEY);

  log("generating changelog from fresh commits");

  const { output, error, provider } = await cohere.run([
    {
      role: "system",
      content: `You are a professional release manager.
        ...
        Important: when replying, you only return markdown and nothing else
      `,
    },
    {
      role: "user",
...
async function addImages(changeLog) {
  try {
    /*
      Image generators
    */
    var stableDiffusion = sdk.model("stabilityai/stable-diffusion-xl-base-1.0");
    var ghibli = sdk.model("nitrosocke/Ghibli-Diffusion");
    var midJourney = sdk.model("prompthero/openjourney-v4");
    var allInOne = sdk.model("PublicPrompts/All-In-One-Pixel-Model");
    var pixelArt = sdk.model("kohbanye/pixel-art-style");
    var dreamShaper = sdk.model("Lykon/dreamshaper-8");
    /*
      Claude
    */
    ...
    */
    ...
    const {
    ...
    } = await geminiPro.run([
      {
        role: "system",
        content:
          "When replying, you only return one image URL and nothing else",
      },
      {
        role: "user",
        content: `
